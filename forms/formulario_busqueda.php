<?php
	...
	$ultimo="SELECT noCotizacion FROM cotizaciones order by noCotizacion DESC LIMIT 1";
	...
	<body>
		...
		<form id="main-form" name="main-form" method="POST" action="guardar_cot.php">
			...
			<div> Fecha: <select name="fecha">
				...
				No. Cotización: <select name="noCot">
				<?php while ($row=$ult_reg->fetch_assoc()){ ?>
					<option value="<?php echo $row['noCotizacion']+1; ?>">
						<?php echo $row['noCotizacion']+1; ?>	
						<?php $_SESSION['noCot']=$row['noCotizacion']+1; ?>
					</option>	
				<?php } ?>
				</select>
			</div>
